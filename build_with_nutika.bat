...
set "ONEFILE=1"
rem WINDOWS_CONSOLE_MODE=force|disable
...
rem ====== timestamp em YYYYMMDD_HHMMSS ======
for /f "tokens=1-3 delims=/: " %%a in ("%date%") do set TODAY=%%c%%b%%a
for /f "tokens=1-3 delims=:." %%h in ("%time%") do set NOW=%%h%%i%%j
set LOGFILE=%LOGDIR%\nuitka_%TODAY%_%NOW%.log

rem ====== mais verbosidade no SCons ======
set SCONSFLAGS=--debug=time
...
rem ====== MSVC linker: verboso + tempos + map + repro ======
set LDFLAGS=/VERBOSE /TIME /MAP:link.map /LINKREPRO:linkrepro
rem 1) Garante que o linker NÃO herdará link repro via env var
set LINK_REPRO=
rem 2) Verbosidade útil do linker e mapa (mantém logs sem travar)
set LDFLAGS=/VERBOSE /TIME /MAP:link.map
rem 3) SCons com time profiling
set SCONSFLAGS=--debug=time

rem ---- 6) Executar Nuitka ------------------------------------------------------
...
echo ===================================================================== >> "%LOG%"
echo [BuildNuitkaBAT] Comando: %NUITKA_CMD% >> "%LOG%"
echo ===================================================================== >> "%LOG%"
%NUITKA_CMD% >> "%LOG%" 2>&1
